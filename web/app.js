...
importBackup(data){try{const incoming=normalizeLibrary(data);capture();library=mergeLibraries(library,incoming);for(const s of incoming.snippets)if(!library.snippets.some(x=>x.id===s.id))library.snippets.push(s);selectNote(library.currentId,{captureCurrent:false});toast('Backup merged into your library');}catch(e){toast(e.message);}},
merge(data){capture();const old=JSON.stringify(current());library=mergeLibraries(library,data);applySettings();if(JSON.stringify(current())!==old){const next=current()?.deletedAt?searchNotes(library.notes,'')[0]:current();if(next)selectNote(next.id,{captureCurrent:false});else newNote();}save();},
